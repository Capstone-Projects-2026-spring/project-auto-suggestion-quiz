...
test('opens edit modal when Edit is clicked', () => {
    ...
    const modalHeader = document.querySelector('.modal-header');
    expect(within(modalHeader).getByText('Edit Problem')).toBeInTheDocument();
...
test('shows error when saving edit with empty title', async () => {
    render(<Dashboard {...defaultProps} />);
    fireEvent.click(screen.getAllByText('Edit')[0]);
    const titleInput = screen.getAllByDisplayValue('Problem Alpha')[0];
    fireEvent.change(titleInput, { target: { value: '' } });
    fireEvent.click(screen.getByText('Save Changes'));
    await waitFor(() => expect(screen.getByText('Title is required.')).toBeInTheDocument());
});

test('calls editProblem API and closes modal on successful save', async () => {
    const { editProblem } = require('../api');
    editProblem.mockResolvedValue({ ...mockProblems[0], title: 'Updated Title' });
    ...
    await waitFor(() => {
        expect(editProblem).toHaveBeenCalledTimes(1);
        expect(document.querySelector('.modal-header')).not.toBeInTheDocument();
    });
});

test('shows error when editProblem API fails', async () => {
    const { editProblem } = require('../api');
    editProblem.mockRejectedValue(new Error('Save failed'));
...
test('opens submissions modal when Submissions is clicked', () => {
    render(<Dashboard {...defaultProps} />);
    fireEvent.click(screen.getAllByText('Submissions')[1]);
...
test('shows submission rows when problem has submissions', () => {
    render(<Dashboard {...defaultProps} />);
    fireEvent.click(screen.getAllByText('Submissions')[2]);
