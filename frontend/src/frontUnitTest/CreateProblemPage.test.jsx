...
test('starts with one blank test case', () => {
    render(<CreateProblemPage {...defaultProps} />);
    goToStep4();
    expect(screen.getAllByPlaceholderText('e.g. add(1, 2)').length).toBe(1);
});
...
test('can add a test case', () => {
    ...
    expect(screen.getAllByPlaceholderText('e.g. add(1, 2)').length).toBe(2);
});
...
test('can remove a test case when more than one exists', () => {
    ...
    goToStep4();
    fireEvent.click(screen.getByText('+ Add Test Case'));
    fireEvent.click(screen.getAllByTitle('Remove test case')[0]);
    expect(screen.getAllByPlaceholderText('e.g. add(1, 2)').length).toBe(1);
});
